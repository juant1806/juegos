...
document.addEventListener("deviceready",function(){
	$("#consulta1").on("tap", function (){
		...
		$.ajax({
			...
			url:"http://192.168.1.71//buscarGenero.php",
		...
		$.ajax({
			...
			for (var x= 0; x<datosconsola.juegos.length; x++)
			{
				$('#contenidocon').append('<div><div><div style="display:inline-block"><img src="http://192.168.1.68/videojuegos/recursos/imagenes/fotos/'+datosconsola.juegos[x].clave_videojuego+'.jpg"></div><div style="display:inline-block"><h1>'+ datosconsola.juegos[x].nombre_videojuego +' </h1><h2> '+datosconsola.juegos[x].genero_videojuego+' </h2></div></div><div><h2> '+datosconsola.juegos[x].clasificacion_videojuego+' </h2><h2> '+datosconsola.juegos[x].consola_videojuego+' </h2></div></div>');
			}
